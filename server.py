...
class Server:
    ...
    async def listen(self):
        ...
        self.server = await asyncio.start_server(self.new_conn, "0.0.0.0", 8123)
        loop.create_task(self._background())
        await websockets.server.serve(self.handle_websocket, "localhost", 8765)
        logging.info("Sunucu bağlantıları kabul etmeye hazır")
    ...
    async def handle_websocket(self, websocket, path):
        ...
        if data["action"] == "get_online":
            await websocket.send(json.dumps({"action": "online", "online": len(self.online)}))
        ...
        if data["type"] == 2:
            ...
            if client.uid in self.msgmeter:
                ...
                if self.msgmeter[client.uid] > 160:
                    ...
                    logging.debug(f"{client.uid} Yasaklandı.")
                    await client.send(
                        [
                            "cp.ms.rsm",
                            {"txt": "Atıldın"},
    ...
    async def auth(self, msg, client):
        ...
        if user_data["prem_time"] != 0 and time.time() - user_data["prem_time"] > 0:
    ...
    async def get_clothes(self, uid, type_):
        ...
        if type_ == 1:
            ...
            for item in clothes:
                if item["clid"]:
                    clths["ccltns"][cur_ctp]["cct"].append(
                        f"{item['id']}:" f"{item['clid']}"
                    )
                else:
            ...
            clths = {"clths": []}
            for item in clothes:
                clths["clths"].append({"tpid": item["id"], "clid": item["clid"]})
        elif type_ == 3:
    ...
    async def get_room_items(self, uid, room):
        ...
        for name in names:
            ...
            try:
                ...
                await self.redis.srem(f"rooms:{uid}:{room}:items", f"{name}_{lid}")
                await self.redis.delete(f"rooms:{uid}:{room}:items:" f"{name}_{lid}")
                continue
            for kek in option:
                if kek == "clrs":
                    item = await self.redis.smembers(
                        f"rooms:{uid}:{room}:" f"items:{name}_{lid}:" f"{kek}"
                    )
                else:
                    item = await self.redis.get(
                        f"rooms:{uid}:{room}:items:" f"{name}_{lid}:{kek}"
                    )
                tmp[kek] = item
            items.append(tmp)
            i = 1
    ...
    async def _background(self):
        while True:
            logging.info(f"Cevrimici oyuncu: {len(self.online)}")
            logging.info(f"Oyundan atıldı: {len(self.kicked)}")
            self.msgmeter = {}
            self.kicked = []
            await self.redis.delete(f"online_tr")
            for uid in self.inv.copy():
                inv = self.inv[uid]
                await self.redis.sadd(f"online_tr", uid)
