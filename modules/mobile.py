...
class Mobile(Module):
    ...
    def __init__(self, server):
        self.server = server
        self.commands = {"mkslf": self.make_selfie, "sma": self.save_mobile_appearance, "gtmms": self.get_messages, "sdmm": self.send_message,
                         "rmmm": self.remove_message, "rmmmsdg": self.remove_dialog, "smad": self.mobilad}
    ...
    async def mobilad(self, msg, client):
            if await self.server.redis.get(f"uid:{client.uid}:smad") == "1":
             newmsg = {"msg": {}}
             newmsg["msg"]["tp"] = "chatTextMessage"
             newmsg["msg"]["tag"] = "mobileAd"
             newmsg["msg"]["sid"] = client.uid
             newmsg["msg"]["msg"] = msg[2]["mad"]
             ...
             for uid in room:
                 try:
                     ...
                     room.remove(uid)
                     continue
                 await tmp.send(["mb.smad", {"mad": msg[2]["mad"], "uid": client.uid}])
                 await tmp.send(["cp.cht.sm", newmsg])   
            else:
               await client.send(["cp.ms.rsm", {"txt": "Bu özellik sadece test kullanıcıları için aktiftir."}])  
