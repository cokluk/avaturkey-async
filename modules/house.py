    ...
    async def room(self, msg, client):
        ...
        if subcommand == "info":
            ...
            try:
                ...
                await client.send(["cp.ms.rsm", {"txt": "Bir hata oluştu, lütfen oyuna tekrar girin"}])
                return
            ...
            if not tmp:
                await client.send(["cp.ms.rsm", {"txt": f"Oda bulunamadı, teknik desteğe yazın ({room_addr})"}])
                return
