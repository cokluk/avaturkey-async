...
class Passport(Module):
    ...
    async def set_trophy(self, msg, client):
        if await self.server.redis.get(f"uid:{client.uid}:plus") != "1":
         if msg[2]["trid"] == "freelanceModerator":
          await client.send(["cp.ms.rsm", {"txt": f"Bu ünvanı kullanmak için plus üye olmanız gerek!"}])
